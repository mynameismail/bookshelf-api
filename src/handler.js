...
const getAllBooksHandler = (request, h) => {
  ...
  if (queryName) {
    ...
    filteredBooks = books.filter((book) => book.name.toLowerCase().includes(queryNameLower));
  } else if (parseInt(queryReading, 10) === 0) {
    filteredBooks = books.filter((book) => book.reading === false);
  } else if (parseInt(queryReading, 10) === 1) {
    filteredBooks = books.filter((book) => book.reading === true);
  } else if (parseInt(queryFinished, 10) === 0) {
    filteredBooks = books.filter((book) => book.finished === false);
  } else if (parseInt(queryFinished, 10) === 1) {
    filteredBooks = books.filter((book) => book.finished === true);
  } else {
    filteredBooks = books;
  }

  const allBooks = filteredBooks.map(({ id, name, publisher }) => ({ id, name, publisher }));
  const response = h.response({
    status: 'success',
    data: {
      books: allBooks,
